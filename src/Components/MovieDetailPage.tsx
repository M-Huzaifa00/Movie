import { Image, Box, GridItem, Heading, SimpleGrid, Spinner, Text } from '@chakra-ui/react';
...
export const MovieDetailPage = ({ id }: Props) => {
    ...
    const { data: Images } = useMovieImage(id)
    ...
    return (
        <>
            ...
            <SimpleGrid padding={{base:2,md:6}} columns={{base:2 , md:4}} spacing={5}>
                {
                    Images?.backdrops.map(({ file_path }) => <Image src={imageRequest(file_path)} />)
                }
            </SimpleGrid>
        </>
    )
